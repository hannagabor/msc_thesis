...
\begin{guess} \label{s:dual}
  ...
  \begin{enumerate}
    ...
    \item For every face $f$ of $G^*$ it contains at least one edge that has exactly one
    endpoint on $f$.
  \end{enumerate}
\end{guess}
...
\begin{proof}
  ...
  that has exactly one endpoint on the face that corresponds to $v$ in the dual graph of $G$.
  Hence $H$ contains at least one edge from each wheel if and only if for every face
  of $G^*$, $H^*$ contains at least one edge that has exactly one endpoint on that face.
\end{proof}
...
\begin{guess}\label{s:dual_comp}
  ...
  \begin{enumerate}
    ...
    \item For every face $f$ of $G^*$ it does not contain all the edges having exactly one
    endpoint on $f$.
  \end{enumerate}
\end{guess}
...
TODO: leaving a face is not the same as having exactly one common vertex. Correct it!
