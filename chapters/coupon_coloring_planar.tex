...
maximal \emph{outerplanar} graphs first.

\begin{definition}
   A graph is \emph{outerplanar} if it has a planar drawing for which all vertices
   belong to the outer face. A \emph{maximal outerplanar} graph is a simple outerplanar graph
...
notions first.
\begin{definition}
  Let $G$ be a maximal outerplanar graph of order $n \ge 3$. The \emph{sun graph} $M(G)$
...
\end{remark}
\begin{definition}
  A vertex $v$ of a maximal outerplanar graph is called a \emph{central vertex} if the
...
\end{proof}
\begin{definition}
  A \emph{generalized sun graph} is a maximal outerplanar graph of order
...
\begin{definition}
  Let $G$ be a maximal outerplanar graph and $i \ge 2$. We say that a $uv$ edge is a \emph{chord}
...
\begin{definition}
  Let $H$ be a hypergraph. The \emph{incidence graph} of $H$ is a bipartite graph with one of
...
\end{definition}
\begin{definition}
  A hypergraph is called \emph{planar}, if its incidence graph is planar.
\end{definition}
\begin{definition}
  A vertex coloring of a hypergraph is \emph{proper} if all its hyperedges contain vertices
