...
we can also say something about graphs with other specific $2$-factors.

\begin{claim}
  Let $G$ be a simple triangulated graph of order at lest $4$. If $G$ has a $2$-factor
...
\begin{proof}
  ...
  $2$-factor form a partially ordered set. Let $l$ denote the the maximum
  number of elements in a chain.

  If $l = 1$, then the claim follows from Theorem \ref{thm:outerplanar}, as each cycle
  forms a $2$-coupon colorable outerplanar graph.

  Suppose $l \ge 2$. Let $C_1, \dots C_t$ be the maximal cycles in the poset defined
  by the $2$-factor. We 
\end{proof}
